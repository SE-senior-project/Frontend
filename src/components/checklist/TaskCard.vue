  ...
  >
    <div
      class="
        ...
        shadow-xl
        rounded-[5px]
        border-solid border-[3px] border-orange-500
      "
    >
      <div class="w-full">
        <div
          @click="selectInTask(task.task_id)"
          class="absolute right-[15px] top-[12px] cursor-pointer"
        >
          <img
...
export default {
  ...
  methods: {
    async selectInTask(selectedTask) {
      ...
      console.log(this.GStore.allList);
      if (this.GStore.allList[0].equal === "equal") {
        this.complete = true;
      } else {
        this.complete = false;
      }
